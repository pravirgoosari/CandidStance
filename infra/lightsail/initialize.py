...
items=[secret('postgres-secrets',{'POSTGRES_USER':'candidstance','POSTGRES_DB':'candidstance','POSTGRES_PASSWORD':password}),secret('candidstance-secrets',{'DATABASE_URL':'postgresql://candidstance:'+password+'@postgres:5432/candidstance','DATABASE_SSL':'false','OPENAI_API_KEY':env['OPENAI_API_KEY'],'GOOGLE_API_KEY':env['GOOGLE_API_KEY']}),{'apiVersion':'v1','kind':'ConfigMap','metadata':{'name':'postgres-schema','namespace':'candidstance'},'data':{'schema.sql':(ROOT/'lib/database/schema.sql').read_text()}}]
